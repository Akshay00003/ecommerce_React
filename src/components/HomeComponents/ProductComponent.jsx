...
const ProductComponent = ({ brandId, catName, filterAmount }) => {
  ...
  useEffect(() => {
    ...
    if (brandId) {
      const Name = brands.find((item) => item._id === brandId);
  ...
  }, [categoryId, brandId, data, product, brands, filterAmount]);
  const findBrand = (id) => {
  ...
  return (
    <div className="w-full h-auto">
      ...
      <CategoryComponent
        ...
        product={product}
      />
      <div className="w-full h-auto p-3 border-2">
        <div>
          <h1>{brandName}</h1>
          <h1>{catName}</h1>
        </div>
        ...
        <div className="w-full h-full grid grid-cols-3 gap-2">
          {filteredData.length > 0 ? (
            filteredData.map((item) => (
              <Link key={item._id} to={`/product/${findBrand(item.brand)}/${generateSlug(item.name)}/${item._id}`}>
                <div
                  key={item._id}
